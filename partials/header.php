...
<header class="topbar">
    ...
    <nav class="desktop-nav" data-main-nav>
        <a class="<?= $currentFile === 'dashboard.php' && !$isAdminPage ? 'active' : '' ?>" href="<?= e(url('dashboard.php')) ?>">Trang chủ</a>
        ...
        <?php if (is_admin()): ?>
            <a class="<?= $isAdminPage ? 'active admin-link' : 'admin-link' ?>" href="<?= e(url('admin/index.php')) ?>">Quản trị</a>
        <?php endif; ?>
    </nav>
    ...
    <div class="topbar-actions">
        <div class="notification-menu" data-notification-menu>
            <button class="notification-trigger" type="button" data-notification-trigger aria-label="Thông báo" aria-expanded="false">
                ...
                <?php if ($notificationUnreadCount > 0): ?>
                    <span class="notification-count"><?= $notificationUnreadCount > 99 ? '99+' : (int)$notificationUnreadCount ?></span>
                <?php endif; ?>
            </button>

            <div class="notification-dropdown" data-notification-dropdown>
                <div class="notification-dropdown-head">
                    <div><b>Thông báo</b><small><?= (int)$notificationUnreadCount ?> chưa đọc</small></div>
                    <a href="<?= e(url('notifications.php')) ?>">XEM TẤT CẢ</a>
                </div>
                            ...
                            <a href="<?= e(url('notification.php?id=' . (int)$notification['notification_id'])) ?>" class="notification-item <?= (int)$notification['is_read'] === 0 ? 'unread' : '' ?>">
                                <span class="notification-item-inner">
                                    ...
                                    <span class="notification-copy">
                                        <b><?= e((string)$notification['title']) ?></b>
        ...
        <a class="twoyears-top-icon <?= $currentFile === 'twoyears.php' ? 'active' : '' ?>" href="<?= e(url('twoyears.php')) ?>" title="Quyền lợi #TWOYEARS" aria-label="Quyền lợi #TWOYEARS">
            <span>#</span><b>2Y</b>
        </a>
        ...
        <div class="account-menu" data-account-menu>
            ...
            <div class="account-dropdown" data-account-dropdown>
                <div class="account-dropdown-head">
                    <span class="account-avatar large"><?= e($initial) ?></span>
                    <div><b><?= e($account['username'] ?? '') ?></b><small>Master Account #<?= (int)($account['account_id'] ?? 0) ?></small></div>
                </div>
